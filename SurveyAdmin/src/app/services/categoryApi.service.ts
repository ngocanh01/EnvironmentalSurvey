...
export class CategoryApiService {
  private BASE_URL: string = 'https://localhost:7077/api/surveytype/';
  constructor(private httpClient: HttpClient) {}
  async findAll() {
    return await lastValueFrom(
      this.httpClient.get(this.BASE_URL + 'findallCate')
    );
  }
  ...
  async delete(id: string) {
    return await lastValueFrom(
      this.httpClient.delete(this.BASE_URL + 'delCate/' + id)
    );
  }
}
